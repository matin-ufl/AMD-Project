...
# Convert second data to minute data (AKA shrink to minute axis)
shrink.to.1m <- function(AC.1s, ID) {
     AC.1s <- append.VM(AC.1s, ID)
     axis1.1m <- dataCollapser(AC.1s, TS = "TimeStamp", by = 60, col = "axis1")
     axis2.1m <- dataCollapser(AC.1s, TS = "TimeStamp", by = 60, col = "axis2")
     axis3.1m <- dataCollapser(AC.1s, TS = "TimeStamp", by = 60, col = "axis3")
     vm.1m <- dataCollapser(AC.1s, TS = "TimeStamp", by = 60, col = "VM")
     steps.1m <- dataCollapser(AC.1s, TS = "TimeStamp", by = 60, col = "steps")
     AC.1m <- data.frame(TimeStamp = axis1.1m$TimeStamp, axis1 = axis1.1m$axis1, axis2 = axis2.1m$axis2, axis3 = axis3.1m$axis3, VM = vm.1m$VM, steps = steps.1m$steps)
}